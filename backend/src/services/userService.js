...
const updateUserNoPass = (id, data) => {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            let checkPass = await bcrypt.compareSync(pass, checkUser.password)
            if (checkPass) {
                await userModel.findByIdAndUpdate(id, {email, name, mssv, phone, date_update: Date.now()})
                resolve({
                    errCode: 0,
...
const changePassword = (id, data) => {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            const comparePass = await userModel.findOne({
                password: oldpass
            })//check mật khẩu cũ khớp
            if (comparePass === null) {
                resolve({
                    errCode: 400,
            ...
            await userModel.findByIdAndUpdate(id, {password:encryptPass, date_update: Date.now()})
            resolve({
                errCode: 0,
...
module.exports = {
    ...
    deleteUser,
    deleteManyUser,
    updateUser, changePassword, updateUserNoPass,
}